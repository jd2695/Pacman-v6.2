 ...
 * @author Admin
 */
public class IDDFS extends Controller<MOVE>{
    private static final int MIN_DISTANCE=20;	//if a ghost is this close, run away
    int depthLimitMax = 5; 
        boolean timedgame;
        public IDDFS(boolean timed){
            super();
            timedgame = timed;
        }
        
	public MOVE getMove(Game game,long timeDue){
            int DEPTHLIM = depthLimitMax;
            int bestdots = 500;
            int besttime = 0;
            ...
            while((
                ...
                for(MOVE m:choices){
                    ...
                    if(bfsStack.isEmpty()){
                        ...
                        DEPTHLIM++;
                        System.out.println(DEPTHLIM);
                        for(MOVE mov:MoveArray){
                            Game s = game.copy();
                            s.advanceGame(mov, new StarterGhosts().getMove(s.copy(), System.currentTimeMillis()+DELAY));
                            bfsStack.push(new BFSNode(mov, s, 0));
                        }
                    }
                }
            }

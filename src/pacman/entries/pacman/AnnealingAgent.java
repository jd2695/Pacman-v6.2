 ...
 * @author Admin
 */
public class AnnealingAgent extends Controller<MOVE>{
    private static final int MIN_DISTANCE=20;	//if a ghost is this close, run away
    private static final int geneDepth = 6;
    boolean timedgame;
    
        public AnnealingAgent(boolean timed){
            super();
            timedgame = timed;
        }
	public MOVE getMove(Game game,long timeDue){
            ...
            if(game.getPossibleMoves(game.getPacmanCurrentNodeIndex(), game.getPacmanLastMoveMade()).length > 1){
                ...
                while((System.currentTimeMillis() < timeDue - 3 && timedgame) || (!timedgame && System.currentTimeMillis() < startTime + 4000)){
                    ...
                    double timeratio = (double)((timeDue - 3) - System.currentTimeMillis()) / 20;
                    double deficitratio = 1- ((double)(candidate.evaluate() - neighbor.evaluate()) / candidate.evaluate());
                    double failchance = timeratio * deficitratio;
                    
                    if(neighbor.evaluate() > candidate.evaluate()){
                        candidate = neighbor;
                        System.out.println("Candidate quality: " + candidate.evaluate());
                    }
                    else if( (double) Math.random() < failchance){
                        //System.out.println(deficitratio);
                        System.out.println("Sometimes we fail, with probability " + failchance);
                        candidate = neighbor;
                        System.out.println("Candidate quality: " + candidate.evaluate());
                    }
                }

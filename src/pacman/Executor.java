...
import pacman.entries.pacman.AnnealingAgent;
import pacman.entries.pacman.BFSAgent;
import pacman.entries.pacman.DFSAgent;
import pacman.entries.pacman.EvolutionAgent;
import pacman.entries.pacman.GeneticAgent;
import pacman.entries.pacman.HillClimbAgent;
import pacman.entries.pacman.IDDFS;
import pacman.entries.pacman.KNNAgent;
import pacman.entries.pacman.PerceptronAgent;
import pacman.entries.pacman.learning.AIWrapper;
import pacman.entries.pacman.learning.DataConverter;
import pacman.entries.pacman.learning.DataDumper;
import pacman.entries.pacman.learning.PerceptronTrainer;
...
{	
	...
	public static void main(String[] args)
	{
		...
		boolean visual=true;
                //exec.runExperiment(new AIWrapper(new BFSAgent(false)), new StarterGhosts(), 3);
                //exec.runExperiment(new AIWrapper(new AStarAgent(false)), new StarterGhosts(), 10);
                //PerceptronTrainer pt = new PerceptronTrainer(new File("trainingData.txt"));
                //pt.writeToFile(new File("perceptronVector.txt"));
                
		//exec.runGameTimed(new PerceptronAgent("perceptronVector.txt"),new StarterGhosts(),visual);
//		exec.runGameTimed(new NearestPillPacMan(),new AggressiveGhosts(),visual);
                exec.runGameTimed(new BFSAgent(true),new AggressiveGhosts(),visual);
                //exec.runGameTimed(new DataDumper(new KeyBoardInput()),new StarterGhosts(),visual);
                //exec.runGame(new AIWrapper(new BFSAgent(false)), new StarterGhosts(), visual, DELAY);
                
                //exec.runExperiment(new AIWrapper(new GeneticAgent(true)), new StarterGhosts(), 5);
                //exec.runExperiment(new AIWrapper(new AnnealingAgent(true)), new StarterGhosts(), 10);
                //DataConverter d = new DataConverter();
                //d.convertData("trainingData.txt");
